...
class CLIP(Backbone):
    ...
    def extract_features_convnext_(self, x):
        out = {}
        x = self.clip_model.visual.trunk.stem(x).detach()
        out['stem'] = x.contiguous().detach() # os4
        for i in range(4):
            x = self.clip_model.visual.trunk.stages[i](x).detach()
            out[f'res{i+2}'] = x.contiguous().detach() # res 2 (os4), 3 (os8), 4 (os16), 5 (os32)
        
        x = self.clip_model.visual.trunk.norm_pre(x).detach()
        out['clip_vis_dense'] = x.contiguous().detach()
        return out