...
def get_classification_logits_reason(x, text_classifier, logit_scale, num_templates=None, per_class_num=None):
    # x in shape of [B, *, C]
    # text_classifier in shape of [num_classes, C]
    # logit_scale is a learnable scalar https://github.com/mlfoundations/open_clip/blob/main/src/open_clip/model.py#L201
    # return: [B, *, num_classes]
    ...
    cur_idx = 0
    for num_t in num_templates: 
        i_logit = pred_logits[:, :, cur_idx: cur_idx + num_t]
        i_logit = rearrange(i_logit, 'b k (c n) -> b k c n', c=per_class_num)   
        final_pred_logits.append(i_logit.max(-1).values)
        cur_idx += num_t
    void_embed = pred_logits[:, :, -1].unsqueeze(-1).expand(-1, -1, per_class_num)

    final_pred_logits.append(void_embed) # the last classifier is for void
    final_pred_logits = torch.stack(final_pred_logits, dim=-1)
    return final_pred_logits
...
@TRANSFORMER_DECODER_REGISTRY.register()
class MultiScaleMaskedTransformerDecoder_Reason(nn.Module):
    ...
    def forward(self, x, mask_features, mask = None, text_classifier=None, num_templates=None, num_perclass_reason=None):
        # x is a list of multi-scale feature
        assert len(x) == self.num_feature_levels
        src = []
        pos = []
        ...
        # prediction heads on learnable query features
        outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[0],
                                                                               text_classifier=text_classifier, num_templates=num_templates,num_perclass_reason=num_perclass_reason)
        predictions_class.append(outputs_class)
        predictions_mask.append(outputs_mask)
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, text_classifier, num_templates, num_perclass_reason):
        decoder_output = self.decoder_norm(output)
        decoder_output = decoder_output.transpose(0, 1)
        mask_embed = self.mask_embed(decoder_output)
        outputs_mask = torch.einsum("bqc,bchw->bqhw", mask_embed, mask_features)
        ...
        maskpool_embeddings = self._mask_pooling_proj(maskpool_embeddings)
        class_embed = self.class_embed(maskpool_embeddings + decoder_output)
        outputs_class = get_classification_logits_reason(class_embed, text_classifier, self.logit_scale, num_templates, num_perclass_reason)
